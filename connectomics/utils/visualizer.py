...
def get_lookup(label):
    ...
    lookup[:, 1] = 0.8
    lookup[:, 2] = 0.8
    return matplotlib.colors.hsv_to_rgb(lookup)


def get_pca_colors(pred, n_components=3):
    # pred: BCDHW
    x = pred.permute(0, 2, 3, 4, 1)
    B, D, H, W, C = x.shape
    y = x.flatten(0, 3).detach().cpu().numpy()

    pca = PCA(n_components=n_components)
    inds = np.random.randint(y.shape[0], size=1000)
    pca.fit(y[inds])
    z = pca.transform(y)
    out = z.reshape(B, D, H, W, n_components)
    m, M = out.max(), out.min()
    out = (out - m) / max(M - m, 1e-5)
    return torch.from_numpy(out).permute(0, 4, 1, 2, 3)
    ...
    def visualize(self, volume, label, output, weight, iter_total, writer,
        ...
        for idx in range(len(self.cfg.MODEL.TARGET_OPT)):
            ...
            if topt[0] == '9': # semantic segmentation
                if dcl:
                    label[idx] = label[idx].cpu().numpy()
                    lookup = get_lookup(label[idx])
                    label[idx] = lookup[label[idx]]
                    label[idx] = torch.from_numpy(label[idx]).permute(0, 4, 1, 2, 3)   # BDHWC -> BCDHW
                    output[idx] = get_pca_colors(output[idx])
                else:
