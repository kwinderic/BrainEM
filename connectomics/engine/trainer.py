    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, sample in enumerate(self.dataloader):
                ...
                dt = t1 - t0
                t0 = tick()
                print('progress: %d/%d batches, total time %.2fs, data load %.2fs, model %.2fs, post process %.2fs' %
                      (i+1, len(self.dataloader), time.perf_counter()-start,
                       dt, t2-t1, t0-t2))

        end = time.perf_counter()
        print("Prediction time: %.2fs, Pure model inference %.2fs" % (end-start, np.sum(ts)))
