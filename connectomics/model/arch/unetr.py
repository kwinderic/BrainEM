...
class UNETR(nn.Module):
    ...
    def __init__(self,
        ...
        # add for non-isotropic patch size
    ...
    def forward(self, x_in):        # x_in: 2, 1, 16, 256, 256;  feat_size: 4, 16, 16;  feature_size: 16;  hidden_size: 768
        x, hidden_states_out = self.vit(x_in)       # x: 2, 1024, 768
        enc1 = self.encoder1(x_in)                  # enc1: 2, 16(fs), 16, 256, 256
        x2 = hidden_states_out[3]
        enc2 = self.encoder2(
            self.proj_feat(x2, self.hidden_size, self.feat_size))   # x2: 2, 1024, 768 => enc2: 2, 32(fs*2), 32, 128, 128
        x3 = hidden_states_out[6]
        enc3 = self.encoder3(
            self.proj_feat(x3, self.hidden_size, self.feat_size))   # x3: 2, 1024, 768 => enc3: 2, 64(fs*4), 16, 64, 64
        x4 = hidden_states_out[9]
        enc4 = self.encoder4(
            self.proj_feat(x4, self.hidden_size, self.feat_size))   # x4: 2, 1024, 768 => enc4: 2, 128(fs*8), 8, 32, 32
        dec4 = self.proj_feat(x, self.hidden_size, self.feat_size)  # dec4: 2, 768(hs), 4, 16, 16
        dec3 = self.decoder5(dec4, enc4)    # dec4: 2, 768(hs), 4, 16, 16;  enc4: 2, 128(fs*8), 8, 32, 32  =>  dec3: 2, 128, 8, 32, 32 
        dec2 = self.decoder4(dec3, enc3)    # dec3: 2, 128, 8, 32, 32;  enc3: 2, 64(fs*4), 16, 64, 64  =>  dec2: 2, 64, 16, 64, 64
        dec1 = self.decoder3(dec2, enc2)    # dec2: 2, 64, 16, 64, 64;  enc2: 2, 32(fs*2), 32, 128, 128  =>  dec1: 2, 32, 32, 128, 128

        # dec1: 2, 32, 32, 128, 128;  enc1: 2, 16(fs), 16, 256, 256  =>  out: 2, 16, 16, 256, 256
        if self.patch_size[0] != self.patch_size[1]:
            dec1 = self.dtrans(dec1)        # 2, 32, 32, 128, 128 -> 2, 32, 8, 128, 128
