...
from .loss import *
from .malis_loss import MalisLoss
    ...
    def evaluate(self,
        ...
        for i in range(self.num_target): # iterate over tasks
            ...
            for j in range(len(self.loss_fn[i])): # iterate over losses for a task
                ...
                # w_mask is None for dice loss
                # if w_mask is not None:
                #     print(w_mask.shape)
                # else:
                #     print(self.loss_fn[i][j])

                # DEBUG: to show weight map
                # import matplotlib.pyplot as plt 
                # w_mask = w_mask[0].cpu().numpy()
                # fig, axs = plt.subplots(4, 4, figsize=(10,10))
                # k = 0
                # for i in range(4):
                #     for j in range(4):
                #         if w_mask.ndim == 4:
                #             axs[i,j].imshow(w_mask[:, k].transpose(1,2,0))
                #         else:
                #             axs[i,j].imshow(w_mask[k])
                #         k += 1
                # plt.savefig(f"temp_{0}.jpg", dpi=128)
