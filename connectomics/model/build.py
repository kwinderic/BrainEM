...
def build_model(cfg, device, rank=None):
    ...
    if model_arch == 'swinunetr':
        ...
        kwargs['patch_size'] = cfg.MODEL.UNETR_PATCH_SIZE    #    
        ...
        cfg_cls = MODEL_MAP['cfg_mixin']
        if issubclass(model_cls, cfg_cls):
            kwargs['use_matchness'] = cfg.MODEL.USE_MATCHNESS
            kwargs = model_cls.parse_config(cfg, kwargs)
    elif 'e2e_mixin' in MODEL_MAP:
        model_cls = MODEL_MAP[cfg.MODEL.ARCHITECTURE]
        e2e_cls = MODEL_MAP['e2e_mixin']
        if issubclass(model_cls, e2e_cls):
            kwargs['use_matchness'] = cfg.MODEL.USE_MATCHNESS
            kwargs = model_cls.parse_config(cfg, kwargs)
