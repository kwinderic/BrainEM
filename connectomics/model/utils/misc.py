...
class LayerNorm3d(nn.Module):
    ...
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_first_v2"):
        super().__init__()
    ...
    def forward(self, x):
        ...
        elif self.data_format == "channels_first_v2":       # faster
            return F.layer_norm(x.permute(0, 2, 3, 4, 1), self.normalized_shape, self.weight, self.bias, self.eps).permute(0, 4, 1, 2, 3)
...
def get_norm_3d(norm: str, out_channels: int, bn_momentum: float = 0.1) -> nn.Module:
    ...
    """
    assert norm in ["bn", "sync_bn", "gn", "in", "none", "ln"], \
