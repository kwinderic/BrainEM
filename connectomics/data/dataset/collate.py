...
class TrainBatch:
    ...
    def _handle_batch(self, pos, out_input, out_target, out_weight, clean_input=None):
        self.pos = pos
        self.out_input = torch.from_numpy(np.stack(out_input, 0))

        if clean_input is not None:
            self.clean_input = torch.from_numpy(np.stack(clean_input, 0))
    ...
    def _pin_batch(self):
        ...
        if hasattr(self, "clean_input"):
            self.clean_input = self.clean_input.pin_memory()
