...
class VolumeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 data_std=0.5,
                 data_match_act='none',
                 ensure_single_connected=False,          # ensure each mask in single connected
                 return_clean_input=False       # whether return the un augmentated inputs
        ):

        self.ensure_single_connected = ensure_single_connected
        self.return_clean_input = return_clean_input
    ...
    def _process_targets(self, sample):
        if self.return_clean_input:
            pos, out_volume, out_label, out_valid, clean_volume = sample
            clean_volume = self._process_image(clean_volume)
        else:
            pos, out_volume, out_label, out_valid = sample

        if self.ensure_single_connected:
            dtype = out_label.dtype
            out_label = out_label.astype(np.int32)
            if out_label.ndim == 3:
                out_label = skimage_label(out_label)
            elif out_label.ndim == 4:
                out_label = np.stack([skimage_label(l) for l in out_label], axis=0)
            out_label = out_label.astype(dtype)
        ...
        if out_label is None: # unlabeled data, compatible with collate_fn_test
            if self.return_clean_input:
                return pos, out_volume, clean_volume
        ...
        if self.return_clean_input:
            return pos, out_volume, out_target, out_weight, clean_volume
    ...
    def _rejection_sampling(self, vol_size):
        ...
        while True:
            ...
            if self.augmentor is not None:
                ...
                random_seed = np.random.randint(0, 1000)        # inherent seed from worker
                augmented = self.augmentor(data, random_seed=random_seed)
            ...
            if self._is_valid(out_valid) and self._is_fg(out_label):
                # ++
                if self.return_clean_input:
                    return pos, out_volume, out_label, out_valid, clean_volume
    ...
    def _assert_valid_shape(self):
        print("sample size:",self.sample_volume_size)
