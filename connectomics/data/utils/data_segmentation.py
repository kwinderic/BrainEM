...
# The above code is importing functions `erosion`, `dilation`, and `disk` from the
# `skimage.morphology` module in Python. These functions are typically used for morphological
# operations on images, such as erosion and dilation, which are common techniques in image processing
# for tasks like noise reduction, edge detection, and image segmentation. The `disk` function is used
# to create a disk-shaped structuring element that can be used in these morphological operations.
...
def seg_to_targets(
    ...
    for tid, topt in enumerate(topts):
        ...
        elif topt[0] == 'X':  # boundary v2
            args = topt.split('-')
            if len(args) == 2:
                _, alpha = args[0], int(args[1])
            elif len(args) == 1:
                alpha = 5
            else:
...
def aff2boundary_v2(affinity, alpha=20):
    log_once(f"Use aff2boundary_v2: alpha={alpha}")
    ...
    boundary_map = weight_function(distance_map)
    return boundary_map
