...
class QueryProposal(nn.Module):
    ...
    def __init__(self, num_features, num_queries, num_classes):
        ...
        # )
        self.conv_proposal_cls_logits = nn.Sequential(
            nn.Conv3d(num_features, num_features, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(num_features, num_classes + 1, kernel_size=1, stride=1, padding=0),
        )
    ...
    def seek_local_maximum(self, x, epsilon=1e-6):
        ...
        """
        x_pad = F.pad(x, (1, 1, 1, 1, 1, 1), "constant", 0)
        # we use 6-connected for 3D 
        maximum = (x >= x_pad[:, :, :-2, 1:-1, 1:-1]) & (x >= x_pad[:, :, 2:, 1:-1, 1:-1]) & \
                  (x >= x_pad[:, :, 1:-1, :-2, 1:-1]) & (x >= x_pad[:, :, 1:-1, 2:, 1:-1]) & \
                  (x >= x_pad[:, :, 1:-1, 1:-1, :-2]) & (x >= x_pad[:, :, 1:-1, 1:-1, 2:]) & \
                  (x >= epsilon)
        return maximum.to(x)

    def forward(self, x, pos_embeddings):

        proposal_cls_logits = self.conv_proposal_cls_logits(x)  # b, c, d, h, w
        proposal_cls_probs = proposal_cls_logits.softmax(dim=1)  # b, c, d, h, w
        # proposal_cls_one_hot = F.one_hot(proposal_cls_probs[:, :-1, :, :].max(1)[1],
        #                                  num_classes=self.num_classes + 1).permute(0, 3, 1, 2)  # b, c, h, w
        proposal_local_maximum_map = self.seek_local_maximum(proposal_cls_probs)  # b, c, d, h, w
        proposal_cls_probs = proposal_cls_probs + proposal_local_maximum_map  # b, c, d, h, w
