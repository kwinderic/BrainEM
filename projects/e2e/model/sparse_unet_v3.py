...
class SparseUNet3DV3(UNet3D, E2EMixin):
    """
    ...
    def __init__(self,
        ...
        for i in range(self.depth):
            ...
            self.down_layers0.append(layer)

        fusion_channel = 64
        fusion_level = 1

        self.fusion_block0 = FusionBlock(
        ...
        for i in range(self.depth):             # the second stage get input from level-1
            if i == 1:
                continue
            ...
            if i == 0:
                channel_pre, channel_post = fusion_channel, filters[1]
            else:
                channel_pre, channel_post = filters[previous], filters[i]
            
            layer = nn.Sequential(
                self._make_pooling_layer(isotropy[i], previous, i),
                conv3d_norm_act(channel_pre, channel_post, kernel_size,
                                stride=stride, padding=padding, **self.shared_kwargs),
                block(channel_post, channel_post, **self.shared_kwargs))
            self.down_layers1.append(layer)
        ...
        self.fusion_block1 = FusionBlock(
            filters, fusion_level, 
            fusion_channel, fusion_channel, 
            kernel_size=(1, 3, 3), padding=(0, 1, 1),
            shared_kwargs=self.shared_kwargs
        ...
        # self.down_layers2 = nn.ModuleList()
        # for i in range(self.depth):             # the second stage get input from level-1
        #     if i == 1:
        #         continue
        ...
        #                         stride=stride, padding=padding, **self.shared_kwargs),
        #         block(channel_post, channel_post, **self.shared_kwargs))
        #     self.down_layers2.append(layer)
        ...
        # self.fusion_block2 = FusionBlock(
        #     filters, fusion_level, 
        #     fusion_channel, fusion_channel, 
        #     kernel_size=(1, 3, 3), padding=(0, 1, 1),
        #     shared_kwargs=self.shared_kwargs
    ...
    def _forward_backbone(self, x):
        ...
        x_fp = []               # feature pyramid
        for i in range(self.depth):
            x = self.down_layers0[i](x)
            x_fp.append(x)

        x_hr = x_fp[0]          # keep and send high resolution feature to each stage

        # stage-1
        x = self.fusion_block0(x_fp)

        x_fp = [x_hr]
        for i in range(self.depth-1):
            x = self.down_layers1[i](x)
            x_fp.append(x)

        x = self.fusion_block1(x_fp)

        # stage-2
        # x_fp = [x_hr]
        # for i in range(self.depth-1):
        #     x = self.down_layers2[i](x)
        #     x_fp.append(x)

        # x = self.fusion_block2(x_fp)
