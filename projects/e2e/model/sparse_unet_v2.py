...
class InstDecoder(nn.Module):
    def __init__(self, fusion_channel, num_iter=2):
        ...
        self.rpn = SparseInstHead(
            num_convs=4, num_masks=100-1, kernel_dim=64, 
        ...
        #         kernel_size=(1, 3, 3), padding=(0, 1, 1))
        self.bg_predictors = nn.ModuleList([
            Predictor(fusion_channel, kernel_dim, norm_kernel=False) for _ in range(num_iter + 1)
        ])
    ...
    def forward(self, x, mask_features):
        ...
        bg_feat = img_features.mean(dim=[2, 3, 4]).unsqueeze(1)         # Bx1xC
        bg_mask = self.bg_predictors[-1](bg_feat, mask_features)
        ...
        for i in range(self.num_iter):
            ...
            bg_mask = self.bg_predictors[i](bg_feat, mask_features)
